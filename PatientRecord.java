...
public class PatientRecord implements Comparable<PatientRecord> {
    ...
    /**
     * Creates a new patient record and assigns it a CASE_NUMBER, as the counter will advance
     * when the
    ...
    /**
     * For tester class purposes only: resets PatientRecord.patientCounter to 1. This method
     * should be called at the beginning of EACH tester method to ensure that
     * the methods are not dependent on being called in a particular order.
     */
     ...
     * @return {@code true} if other is instanceof PatientRecord and this PatientRecord and
     * other have
    ...
    public int compareTo(PatientRecord other) {
        ...
        if (this.triage == other.triage) {
            return Integer.compare(this.orderOfArrival, other.orderOfArrival);
        } else {
